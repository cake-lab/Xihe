...
def index_points(points, idx):
    ...
    B = points.shape[0]
    view_shape = [1 for _ in idx.shape]
    view_shape[0] = B
...
def farthest_point_sample_fast(xyz, npoint: int):
    """
    Input:
        xyz: pointcloud data, [B, N, C]
        npoint: number of samples
...
def knn_point(nsample: int, xyz, new_xyz):
    """
    Input:
...
def sample_and_group(npoint: int, nsample: int, xyz, points):
    """
    Input:
        npoint:
        nsample:
...
def sample_and_group_ds(npoint: int, nsample: int, xyz, points, density_scale):
    """
    Input:
        npoint:
        nsample:
...
def sample_and_group_all_ds(xyz, points, density_scale):
    """
    Input:
        xyz: input points position data, [B, N, C]
        points: input points data, [B, N, D]
...
def compute_density(xyz, bandwidth: float):
    '''
    xyz: input points position data, [B, N, C]
    '''
    #import ipdb; ipdb.set_trace()
...
class DensityNet(nn.Module):
    ...
    def forward(self, density_scale):
        ...
        for i, v in enumerate(zip(self.mlp_convs, self.mlp_bns)):
            conv, bn = v
...
class PointConvDensitySetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        if self.group_all:
            new_xyz, new_points, grouped_xyz_norm, grouped_density = sample_and_group_all_ds(
                xyz, points, inverse_density.view(B, N, 1))
        else:
            new_xyz, new_points, grouped_xyz_norm, _, grouped_density = sample_and_group_ds(
        ...
        new_points = torch.matmul(
            ...
            weights.permute(0, 3, 2, 1)).view(B, self.npoint, -1)
        new_points = self.linear(new_points)
        new_points = self.bn_linear(new_points.permute(0, 2, 1))
        new_points = F.relu(new_points)
        new_xyz = new_xyz.permute(0, 2, 1)
